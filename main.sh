...
if [ -f /etc/os-release ] && grep -q 'Arch Linux' /etc/os-release; then
    echo "Sadly this script does not currently support Arch Linux but m working on it."
fi

if [ -f /etc/os-release ] && grep -qi 'debian' /etc/os-release; then
    echo "Debian detected, runnning scripts."
    sudo apt install zsh
    sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"
    echo "do you want to have my config (Y/n)"
    read configresponse
    if [ $configresponse = "y" || $configresponse = "Y" ] then
